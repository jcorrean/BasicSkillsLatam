...
In this first model we examine the propensity of connections between programs and basic skills compared with the exogeneous factor that considers the brochure length. Based on our descriptive findings, we expect to find a negative and statistically significant effect for the term of network density. This is our baseline model that allows us to compare other effects in other ERGM models. In this first model, we also examine the effect of other hidden factors that might be confounded with the length of program brochures. Given the weak connectivity observed for the connection between program brochures and basic skills, we are inclined to think that program brochures might exert a positive and statistically significant effect on the observed network ties.

posit that the top five most central basic skills suffices to account for the connectivity observed for our bipartite network in the region. When each of these skills are modeled as statistical terms, their estimates will be positive, greater than zero, and statistically significant. 
...
The results of our first model confirms our expectations regarding the significance of these basic skills for accounting for the network connectivity between programs and basic skills in the Latin American academic offering. In order to test the robustness of these basic skills, we introduce other factors in subsequent models.

### **Model 2** 

In this second model, we also include the effects for doctoral and master's programs.  Given the weak connectivity reported in the descriptive findings, we expect that the effects of graduate programs should be negative and statistically significant.
...
In this model, we want to test the effects of other confounding factors that might be hidden behind the length of the brochure that is used to promote the programs where these skills are explicitly mentioned. We claim that this covariable can hide confounding factors associated to the skills targeted by the programs because the information contained in them can describe other attributes of the program that have nothing to do with basic skills formation, like the possibility to getting a double degree or a professional certification. We expect that the statistical estimates for our five basic skills will remain positive, greater than zero, and statistically significant, while the statistical estimate for the brochure length will remain non-significant and close to zero.
